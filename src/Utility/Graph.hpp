...
class Graph {
    ...
    friend class Utility::BFSHelper;
...
public:
    ...
    /**
     * @brief Initializer => Vex_Only
     *
     */
    explicit constexpr Graph(
        const VertexList& VexInit,
        const bool&       if_weighted = false,
        const bool&       if_directed = false
    ) {
        // 0. set type
        this->if_weighted = if_weighted;
        this->if_directed = if_directed;
        // 1. import vertex
        const int& num_of_v = VexInit.size();
        size                = num_of_v;
